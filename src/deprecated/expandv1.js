...
import { rotate } from "./rotationUtils.js";
...
function animate() {
  ...
  for (let i = 1; i <= groupNum; i++) {
    ...
    const motionScale = fixedAmplitude * (wave * 0.3 + Math.abs(wave) * 0.7);
...
function createGradientCube(position) {
  ...
  const gradientIndex = 5; // Or pick randomly, or by group
  ...
  for (let i = 0; i < geometry.attributes.position.count; i++) {
    ...
    pos.add(position); // <-- This is the world position of the cube
