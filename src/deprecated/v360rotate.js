...
import { rotate360 } from "./rotationUtils.js";
...
let faceCycle = ["top", "right", "bottom", "left"];
let faceIndex = 0;

window.addEventListener("keydown", (e) => {
  if (e.code === "Space" && !isRotating) {
    const face = faceCycle[faceIndex];
    faceIndex = (faceIndex + 1) % faceCycle.length;

    isRotating = true;
    rotate360(face, groupArray, "clockwise", () => {
      isRotating = false;
    });
  }
});
